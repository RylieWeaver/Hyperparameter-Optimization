...
This file is made to be applicable to the large majority of models by using common hyperparameters to vary. The user is encouraged to adapt this file depending on the model and their desired hyperparameters of study. It consists of a list of JSON dictionaries, each one of which defines a hyperparameter. Each dictionary has the following required keys:
...
** `constant`:
  * Each model is initialized with the same specified value.
...
** `int`:
  ...
  * Mutation is performed by adding the results of a random draw from
...
The following keys are required depending on the value of the `type` key.
...
* `sigma`: the sigma value used by the mutation operator.
